/**
 * Fetches an Apex quality profile from SonarQube and converts it to apexAnalyzer.rules format.
 */

const APEX_LANGUAGE = 'apex';
const REPO_KEY = 'qualimetry-apex';
...
export async function fetchQualityProfiles(config: SonarConfig): Promise<QualityProfile[]> {
    ...
    const url = `${base}/api/qualityprofiles/search?language=${encodeURIComponent(APEX_LANGUAGE)}`;
    const baseHeaders: Record<string, string> = {
        'Accept': 'application/json',
        'User-Agent': 'Qualimetry-Apex-Extension/1.0',
    };
