...
Powered by the same analysis engine as the [Qualimetry Apex Analyzer for SonarQube](https://github.com/Qualimetry/sonarqube-apex-plugin) and the [Qualimetry Apex Analyzer for IntelliJ](https://github.com/Qualimetry/intellij-apex-plugin).
...
- **Import from SonarQube** — pull your Apex quality profile (rules and severities) into workspace or user settings; the imported rules control which checks run. URL and profile remembered; token stored in VS Code's secret storage. Rules auto-resync from the same server on startup.
...
Install from the [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=Qualimetry.qualimetry-vscode-apex-plugin) or the [Open VSX Registry](https://open-vsx.org/extension/Qualimetry/qualimetry-vscode-apex-plugin) and ensure Java 17+ is available.
...
| `apexAnalyzer.sonar.autoSyncOnStartup` | boolean | `true` | Automatically re-import rules from the last-used SonarQube server when the extension starts. |
...
The last-used URL and profile are remembered (in the extension's global state, not in settings). Your token is stored in VS Code's encrypted **secret storage** so the automatic re-sync can authenticate; leave the token prompt empty to remove a stored token.

After the first successful import, the extension automatically re-imports the rules from the same server and profile each time it starts, keeping the editor aligned with your SonarQube quality profile. If the server is unreachable, the previously imported rules stay in effect. Disable this with `apexAnalyzer.sonar.autoSyncOnStartup: false`. To switch back to the extension default and only override a few rules, set `apexAnalyzer.rulesReplaceDefaults` to `false`.
