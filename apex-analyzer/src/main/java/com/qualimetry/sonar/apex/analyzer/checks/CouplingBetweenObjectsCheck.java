...
public class CouplingBetweenObjectsCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "threshold",
            description = "Maximum number of distinct types a class may depend on",
    ...
    public void scan(InputFile file, String content) {
        ...
        if (referencedTypes.size() > threshold) {
            addIssue(1, "Class has too many dependencies on other types (" + referencedTypes.size() + ", max " + threshold + ").");
        }
    }
}
