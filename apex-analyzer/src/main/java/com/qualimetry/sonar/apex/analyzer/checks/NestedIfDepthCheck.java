...
public class NestedIfDepthCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "maxDepth",
            description = "Maximum allowed nesting depth for if statements",
    ...
    public void scan(InputFile file, String content) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (ifOnLine) {
                ...
                if (nesting > maxDepth && issueLine < 0) {
                    issueLine = i + 1;
                }
            }
        ...
        if (issueLine > 0) {
            addIssue(issueLine, "If statements must not be nested beyond depth " + maxDepth + ".");
        }
    }
}
