...
public class SwitchDensityCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "threshold",
            description = "Maximum number of statements allowed per switch case",
    ...
    public void scan(InputFile file, String content) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (inSwitch && depth >= switchDepth) {
                ...
                if (statementsInCase > threshold && caseLine > 0) {
                    addIssue(caseLine, "Switch case has " + statementsInCase + " statements (max " + threshold + ").");
                }
                statementsInCase = 0;
                caseLine = i + 1;
