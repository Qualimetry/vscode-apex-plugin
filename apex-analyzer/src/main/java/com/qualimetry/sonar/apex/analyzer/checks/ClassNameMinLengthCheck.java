...
public class ClassNameMinLengthCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "minLength",
            description = "Minimum number of characters allowed in a class name",
    ...
    public void scan(InputFile file, String content) {
        ...
        while (m.find()) {
            if (m.group(1).length() < minLength) {
                addIssue(lineOf(content, m.start()), "Class name must be at least " + minLength + " characters.");
            }
        }
    }
