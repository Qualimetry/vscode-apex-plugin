...
public class CommentDensityCheck extends BaseCheck {
    ...
    private static final int DEFAULT_MIN_DENSITY_PERCENT = 10;

    @RuleProperty(
            key = "minCommentDensity",
            description = "Minimum percentage of comment lines relative to total lines",
            defaultValue = "" + DEFAULT_MIN_DENSITY_PERCENT)
    private int minCommentDensity = DEFAULT_MIN_DENSITY_PERCENT;

    public void setMinCommentDensity(int minCommentDensity) {
        this.minCommentDensity = minCommentDensity;
    }
    ...
    public void scan(InputFile file, String content) {
        ...
        if (total > 0 && codeLines > 5) {
            ...
            if (ratio < minCommentDensity / 100.0) {
