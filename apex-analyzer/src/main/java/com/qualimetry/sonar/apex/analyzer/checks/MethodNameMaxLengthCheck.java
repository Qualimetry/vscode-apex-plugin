...
public class MethodNameMaxLengthCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "maxLength",
            description = "Maximum number of characters allowed in a method name",
    ...
    public void scan(InputFile file, String content) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (m.find() && m.group(1).length() > maxLength) {
                addIssue(i + 1, "Method name exceeds maximum length of " + maxLength + ".");
            }
        }
    }
}
