 ...
 */
@Rule(
        key = "qa-error-handling-no-throw-in-finally",
        name = "Finally blocks must not throw exceptions",
        description = "Throwing from a finally block discards any exception from the try or catch block, making debugging extremely difficult",
        tags = {"error-handling", "cert"},
        priority = Priority.CRITICAL
)
public class NoThrowInFinallyCheck extends BaseCheck {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (inFinally) {
                ...
                if (THROW.matcher(line).find()) {
                    addIssue(i + 1, "Finally blocks must not throw exceptions.");
                }
                if (braceCount <= 0) {
                    inFinally = false;
                }
