...
public class NcssConstructorCheck extends BaseCheck {
    ...
    @RuleProperty(
            key = "threshold",
            description = "Maximum non-commenting source statements allowed in a constructor",
    ...
    public void scan(InputFile file, String content) {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (depth == 1 && constructorStart > 0) {
                if (ncss > threshold) addIssue(constructorStart, "Constructor NCSS is " + ncss + " (max " + threshold + ").");
                constructorStart = -1;
                ncss = 0;
            }
        }
