...
import com.qualimetry.sonar.apex.analyzer.checks.RuleHelpUrls;
...
public final class DiagnosticMapper {
    ...
    /**
     * Converts a {@link BaseCheck.Issue} to an LSP {@link Diagnostic} with default severity.
     */
    public static Diagnostic toDiagnostic(BaseCheck.Issue issue) {
        return toDiagnostic(issue, null);
    }
     ...
     */
    public static Diagnostic toDiagnostic(BaseCheck.Issue issue, LspConfig config) {
        Diagnostic d = new Diagnostic();
        d.setMessage(issue.message());
        d.setCode(issue.ruleKey());
        d.setSource("qualimetry-apex");
        d.setCodeDescription(new DiagnosticCodeDescription(RuleHelpUrls.helpUrl(issue.ruleKey())));
        d.setSeverity(config != null
                ? config.getSeverity(issue.ruleKey())
                : SeverityMap.getSeverity(issue.ruleKey()));
