...
class DiagnosticMapperTest {
    ...
    @Test
    void codeDescriptionLinksToRuleDoc() {
        BaseCheck.Issue issue = new BaseCheck.Issue(1, "msg", "qa-security-no-system-debug");

        Diagnostic diag = DiagnosticMapper.toDiagnostic(issue);

        assertThat(diag.getCodeDescription().getHref())
                .isEqualTo("https://github.com/Qualimetry/sonarqube-apex-plugin/blob/main/docs/rules/qa-security-no-system-debug.md");
    }
